...
def main():
    ...
    total_mass = xp.sum(mass)
    cm_position = xp.sum(mass[:, xp.newaxis] * position, axis=0) / total_mass
    cm_velocity = xp.sum(mass[:, xp.newaxis] * velocity, axis=0) / total_mass


    position = position - cm_position
    velocity = velocity - cm_velocity



    scene = canvas(width=1080, height=600)
    running = True
    ...
    toggle_button = button(text='Toggle Simulation', bind=lambda:toggle_simulation())


    momentum_graph = graph(title="總動量隨時間變化(質心座標系)", xtitle="時間 (秒)",
    ...
    while day < max_day:
        ...
        total_momentum_vector = xp.sum(mv, axis=0)
        total_momentum = xp.linalg.norm(total_momentum_vector)
...
def euler_integration(mass, position, velocity, dt, xp, softening):
    G = 6.67430e-11
    a = getAcc(mass, position, softening, xp, G)
    velocity += a * dt
